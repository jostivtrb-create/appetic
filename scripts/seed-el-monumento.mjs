// 🌱 Alta del local "El Monumento" en Firestore.
// Uso: node scripts/seed-el-monumento.mjs
// Requiere: scripts/serviceAccount.json (llave de servicio de Firebase, NO se sube a git).
//
// Los datos del menú viven en src/dev/elMonumento.js (fuente única, compartida con ?preview=1).
...
import { SLUG, ADMIN_EMAIL, MONUMENTO_LOCAL, MONUMENTO_PRODUCTOS } from '../src/dev/elMonumento.js'
...
async function run() {
  const { id, ...localData } = MONUMENTO_LOCAL
  ...
  localData.destacadosHome = computeDestacadosHome(MONUMENTO_PRODUCTOS)
  ...
  console.log(`✓ Local ${prev.exists ? 'actualizado' : 'creado'}: locales/${SLUG} (El Monumento)`)
  ...
  const nuevosIds = new Set(MONUMENTO_PRODUCTOS.map(p => p.id))
  ...
  for (const p of MONUMENTO_PRODUCTOS) {
    const { id: pid, ...data } = p
    await prodsRef.doc(pid).set(data) // sin merge: reemplaza el doc completo
  }
  console.log(`✓ ${MONUMENTO_PRODUCTOS.length} productos cargados`)
