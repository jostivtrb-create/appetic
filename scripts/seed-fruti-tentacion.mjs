// 🌱 Alta del local "Fruti Tentación" en Firestore.
// Uso: node scripts/seed-fruti-tentacion.mjs
// Requiere: scripts/serviceAccount.json (llave de servicio de Firebase, NO se sube a git).
//
// Los datos del menú viven en src/dev/frutiTentacion.js (fuente única, compartida con ?preview=1).
...
import { SLUG, ADMIN_EMAIL, FRUTI_LOCAL, FRUTI_PRODUCTOS } from '../src/dev/frutiTentacion.js'
...
async function run() {
  const { id, ...localData } = FRUTI_LOCAL
  ...
  localData.destacadosHome = computeDestacadosHome(FRUTI_PRODUCTOS)
  ...
  console.log(`✓ Local ${prev.exists ? 'actualizado' : 'creado'}: locales/${SLUG} (Fruti Tentación)`)
  ...
  const nuevosIds = new Set(FRUTI_PRODUCTOS.map(p => p.id))
  ...
  for (const p of FRUTI_PRODUCTOS) {
    const { id: pid, ...data } = p
    await prodsRef.doc(pid).set(data) // sin merge: reemplaza el doc completo
  }
  console.log(`✓ ${FRUTI_PRODUCTOS.length} productos cargados`)
