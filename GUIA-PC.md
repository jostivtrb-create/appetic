...
## 3) 🗄️ Índices de Firestore para el panel del domiciliario (un clic)

El buscador por código y el aviso de pedido nuevo usan consultas **collectionGroup** sobre
`pedidos`. La **primera vez** que se usen, Firestore mostrará en la consola del navegador un
error con un **link "create index"** → ábrelo y dale crear (2 índices como máximo: `codigo`
y `createdAt` en alcance *collection group*). Tarda 1-2 min en construirse.
...
- **🛵 Domiciliarios** (bloque de arriba): agrega el/los correos Google de los repartidores.
  Su panel es **`/domiciliario`** (global): escriben el código del pedido y ven el detalle
  con el precio real. Les avisa con sonido cuando entra un domicilio nuevo.
...
4. Entra a `/domiciliario` con un correo de la lista → escribe el código → debe mostrar
   el pedido con el precio real (y sonar si entra otro pedido mientras está abierto).
